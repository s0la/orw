	...
	case $property in
		...
		*)
			if [[ $property == active && $(which convert) ]]; then
				magick -size 2x2 xc:$color ${gtk_conf%/*}/apps/assets/active.png
				magick -size 2x2 xc:$(offset_color $color ${secondary_shade_offset-+30}) ${gtk_conf%/*}/apps/assets/hover.png
			fi
