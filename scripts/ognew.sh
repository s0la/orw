...
if ((mono)); then
	...
	#echo $avg_value
	final_accents=( ${frequent_accents[*]} )
	...
	sort_accents() {
		...
		#for c in ${!light_accent_colors[*]}; do
		#	print_color $c label
		#done
		#exit
		...
		darkest=$(tr ' ' '\n' <<< ${light_accent_colors[*]} | 
			grep -v $most_vibrant | sort -t ';' -nk 4,4 -nk 3,3r |
		...
		(
			...
			#echo $darkest
				...
				function get_color_old(color, second) {
					get_rgb(color)
					rgb_to_xyz(r, g, b)
					xyz_to_lab(X, Y, Z)
			...
			function get_color3(ar, del) {
				...
				for (i in ac) {
					...
					for (ai in ar) {
						...
						if ((del || ai < 5) && ac[i] != ar[ai]) {
							...
							#if (fai == 2) print lab, ac[i], ar[ai]

							if (del && ! dl[lab]) { dl[lab] = 1; avl += lab }
							#if (del && ! dl[lab]) { dl[lab] = 1; avl += lab; print lab, ac[i], ar[ai] }


							if (lab < mlab) mlab = lab
							#if (del) continue

							#if (del) print lab, ds, ac[i], ar[ai], dl[lab], (lab in dl)
							...
							if (del && int(lab) < ds) {
								...
								#system("~/.orw/scripts/notify.sh " ar[(acp[4] > arp[4] && ai > 1) ? ai : i])
						...
						#if (del) for (l in dl) print ac[i], ar[ai], l, dl[l]
					}
					...
					if (lc) {
						...
						if (al > mal && !(ac[i] in sa)) {
							#print al, mal, i, ac[i], ac[i] in sa
							#if (fai == 2) print al, tl, lc, mal, i, ac[i], ac[i] in sa
							if (fai == 2) slab = al
							if (fai == 2) slab = tl
							if (fai == 2) {
								split(ac[i], acp, "[;_]")
				...
				if (length(del)) return
					...
					if (la && fai == 2 && !skipped) {
						...
						scc = (la < 33 || slab > 220) ? 5 : (slab > 180) ? 3 : 4
						#print slab, slab / scc, la, sqrt((int(slab / scc) - int(la)) ^ 2) #< 5
				...
				for (i in nac) {
					...
					for (ai in ar) {
						...
						if ((del || ai < 5) && nac[i] != ar[ai]) {
							...
							if (!del && similar(nac[i], ar[ai]) && !(nac[i] in sc)) sc[nac[i]] = length(aac)
			...
			END {
				...
				#dsladl = sprintf("%0.f", length(dslad) / 2)
				#if (dsladl % 2) dsladl += 0.5
				dsladl = int(length(dslad) / 2) + 0
				si = int((length(dslad) - dsladl) / 2) + 0

				#print dsladl, si

				#for (z in dslad) {
				#	print z, slad[dslad[z]]
				#}
				...
				for (z=1; z<=dsladl; z++) {
					nac[z + 1] = slad[dslad[si + 1 + z]]
					#print z, nac[z + 1]
				}

				#exit

				#for (z=1; z <= int(length(dslad) / 2 + 1); z++) {
				#	ac[z] = slad[dslad[z]]
				#	print z, ac[z]
				#	#print dslad[z], slad[dslad[z]]
				#	#print lad[z], z | "sort -nk 1,1"
				#}

				#print "HERE"
				#for (z in nac) print nac[z]
				#print "HERE"
				...
				for (z in aac) print aac[z]
				print dac[3]
				print dac[1]
				#for (di in dac) print dac[di] | "sort -r"
