...
yet_another_sort() {
	convert $1 -scale 50x50! -depth 8 +dither -colors 25 -format "%c" histogram:info: |
...
read dbg average_hue_step saturation value colors <<< $(yet_another_sort $1)
declare -A colors
eval colors=( $colors )
		...
		awk -F ';' '
			...
			function add_accent() {
				...
				if (s + v > 40) {
					for (cfa in fa) {
						...
						if (lab < 18 && sqrt((h - mch) ^ 2) < 60) {
							if (fa[cfa] < msv) {
								#print "DEL", fa[cfa], cfa, mc
								delete fa[cfa]
								delete as[cfa]
								delete av[cfa]
								delete asv[cfa]
			...
			END {
				...
				for (ci in sc) {
					...
					skip = s >= 5 && !(d > 30 && v > s && s > 65) &&
						!(s < 20 && v < 20) &&
						v > 10 && v <= 95 && d <= 70

					#if (h >= 160 && h <= 250) print "C:", c, mc, msv
					...
					if (ps > 5 && ps < 85 &&
						pv > 15 && pv < 90 &&
						msv < ps + pv) {
							#if (ph > 250) print "INNN", ph, c, nc, mc
							msv = ps + pv
							mc = c
					}

					#if (ph > 200) print "HERE", ph, h, c, nc, mc

					if (lab > 22 || h - ph >= 50) {
						#if (ph > 200) print "IN", lab, c, cn, mc, msv
						if (mc) add_accent()
					}
				}
				...
				print -1, int(tsv / ac), int(ts / ac), int(tv / ac)
			}'
}
...
while read c; do
	...
	[[ $c != -* ]] &&
		accents+=( $c ) #&&
		#print_color $c label #||
		#read _ avg_{sv,value,saturation} <<< "$c"
done <<< $(get_accents | sort -n)
...
sort_light_accents() {
	...
	(
		...
		#tr ' ' '\n' <<< ${light_accent_colors[*]} | awk -F '[;_]' '
		#	!/'"$most_vibrant"'/ { ac[$5 + $6 + $7] = $0 }
		#	END { for (a in ac) print ac[a] }'

		echo -e "${darkest/ /\\n}"
		tr ' ' '\n' <<< ${light_accent_colors[*]} |
			grep -v "$most_vibrant\|${darkest/ /\\|}$skip" |
			sort -t ';' -nrk 4,4
	) | awk '
			function rgb_to_xyz(r, g, b) {
				R = r / 255
...
echo $avg_sv, $avg_value, $avg_saturation
#exit
...
sort_accents() {
	...
	(
		...
		function get_color3(ar, del) {
			...
			for (i in ac) {
				#print "GEF:", length(ac), length(ar), ac[i]
				...
				for (ai in ar) {
					...
					if ((del || ai < 5) && ac[i] != ar[ai]) {
						...
						if (!del && similar(ac[i], ar[ai]) && !(ac[i] in sc)) sc[ac[i]] = length(aac)

						#if (del && similar(ac[i], ar[ai])) {
						if (del && int(lab) < ds) {
							#print "HERE", i, ac[i], ai, ar[ai], aac[6], lab, ds
							#ar[ai] = aac[6]
							#da[aac[6]] = 1
							#delete aac[6]
							#delete ac[i]
							...
							#delete ar[ai]
							#system("~/.orw/scripts/notify.sh " ar[(acp[3] > arp[3] && ai > 1) ? ai : i])
							#delete ar[(acp[3] > arp[3] && ai > 1) ? ai : i]

							#di = (acp[3] < arp[3]) ? ai : i

							#print "SIM:", ac[i], ar[ai], similar(ac[i], ar[ai])

								#print "REPLACE: ", (acp[4] < arp[4]) ? ac[i] : ar[ai]
								#print "RPC: ", ac[1]
								#print "DELETE: " lab, ac[i], ar[ai], (acp[3] < arp[3]) #? ai : i

							#if (i == 1) {
							#	aac[1] = (acp[4] < arp[4]) ? ac[i] : ar[ai]
							#	delete ac[i]
							#	delete ar[ai]
							#} else delete ar[(acp[3] < arp[3]) ? ai : i]

							#delete ar[(acp[3] > arp[3] && ai > 1) ? ai : i]
							delete ar[(acp[4] > arp[4] && ai > 1) ? ai : i]
				...
				#print length(ac), length(ar) #, ac[i], ar[ai]
				#if (del) continue
				#if (skip) continue
				...
				#print "END: " length(ac), length(ar) #, ac[i], ar[ai]
				#for (z in ac) print z, ac[z]
			}

			#print "FINAL: " length(ac), length(ar)
			#for (z in ac) print z, ac[z]
		...
		END {
			...
			if ((mlab > 12 || mlab < 5) && length(ac) > 3) get_color3(aac)
			for (fai=2; fai<5; fai++) aac[fai] = get_color3(aac)
			...
			for (ai=1; ai<=6; ai++) if (aac[ai]) print aac[ai]
		}'
}
...
# NEWWWW
accent_colors=( $(sort_light_accents) )
#echo ${#accent_colors[*]}
...
read {rgb,hex}_a5_dr <<< $(get_sbg $hex_a5 -13)
read {rgb,hex}_a5_br <<< $(get_sbg $hex_a5 +7)
...
set_bar() {
	...
	[[ ! $single_bg ]] &&
		local bar_bg="#$transparency${hex_pbg#\#}" ||
		local single_hex_bg="#$transparency${hex_bg#\#}" single_hex_fg=$hex_fg
	read {rgb,hex}_abg <<< $(get_sbg $hex_bg +7)
	...
	cat <<- EOF
		...
		bbg ${single_hex_bg:-$bar_bg}
		jbg ${single_hex_bg:-$hex_bg}
		jpfg ${single_hex_fg:-$hex_jpfg}
		jsfg ${single_hex_fg:-$hex_jsfg}
		pbg ${single_hex_bg:-$hex_bg}
		pfg ${single_hex_fg:-$hex_fg}
		sbg ${single_hex_bg:-$hex_bg}
		sfg ${single_hex_fg:-$hex_pfg}
		pbefg $hex_a1
		pbfg $hex_pfg
...
read {rgb,hex}_rofi_sbg <<< $(get_sbg $hex_sbg +2)
...
set_bash() {
	[[ $1 ]] && local type=rgb
	#eval "color_string=\$(cut -d ' ' -f 1,2,4,5,9,10 <<< \${!${type:-rgb}*} |
	eval "color_string=\$(cut -d ' ' -f 1,2,4,5,11,12,14,15 <<< \${!${type:-rgb}*} |
		sed 's/\(^\| \)/ \$/g')"
	eval "bash_colors=( $color_string )"
...
set_rofi() {
	cat <<- EOF
		bg: $hex_rofi_bg;
		tbg: argb:00${hex_rofi_bg#\#};
		//mbg: argb:f5${hex_rofi_bg#\#};
		//msbg: argb:70${hex_rofi_sbg#\#};
		mbg: argb:f0${hex_bg#\#};
		msbg: argb:70${hex_rofi_sbg#\#};
		fg: $hex_sfg;
		bc: $hex_a5;
...
set_ncmpcpp() {
	...
	[[ $1 ]] &&
		...
		awk -i inplace '
			...
			#/prefix|(window|volume|(status|progress)bar)_color/ {
				#gsub("[0-9]+", (/playing/) ? "'$hex_pfgi'" : "'$hex_pbgi'")
			#}
