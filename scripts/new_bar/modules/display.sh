...
get_display() {
	#read display{_count,} <<< $(awk '
	#read {prev,current,next}_display <<< $(awk '
	read {{prev,next}_,}display display_count <<< $(awk '
		...
		#} END { print d, cd }' ~/.config/{openbox/rc.xml,orw/config})

	[[ $display_icons == only ]] &&
		icon=$(get_icon "display_$display") ||
...
check_display() {
	...
	if ((display_count > 1)); then
		...
		#while inotifywait -me close_write --format '%e %w%f' ~/.config/openbox; do
...
#echo $prev_display, $display, $next_display

#inotifywait -me close_write --format '%e %w%f' ~/.config/openbox
