	...
	case $flag in
		...
		s)
			add_wallpaper() {
				...
				if (($1 <= arg_count)); then
					read belong_to_path wallpaper_directory <<< \
						"$(awk '{
							dd = "'$directory_depth'"
							d = "'"$directory"'"; dp = d
							gsub("'\''", "'\''?", dp)
				...
				#((belong_to_path)) || $wallpaper =~ ^\# ]] &&
				[[ $belong_to_path -gt 0 || $wallpaper =~ ^# ]] &&
					write_wallpapers "$wallpaper" $((${display_number:-1} + $1))
			}
			...
			if ((wallpaper_count < display_count)); then
				for display_number in $(seq $((wallpaper_count + 1)) $display_count); do
					wallpapers[display_number]="${wallpapers[*]: -1}"
				done
			fi
...
if [[ $desktop ]]; then
	...
	for wallpaper_index in "${!wallpapers[@]}"; do
		...
		if [[ ${wallpaper//\"/} =~ ^# ]]; then
			...
			wallpapers_to_set+="--output ${displays[wallpaper_index]} --zoom "$wallpaper_path" "
		fi
	done
	...
	eval "xwallpaper $wallpapers_to_set"
